...
vi.mock('sonner', () => ({ toast: { error: vi.fn(), warning: mock.toastWarning } }));
...
it('keeps the latest pick when an earlier clip finishes probing last', async () => {
  ...
  mock.probes.get('first.wav')!(40);
  await new Promise((resolve) => setTimeout(resolve, 0));

  expect(onAccept).toHaveBeenCalledOnce();
  expect(onAccept.mock.calls[0][0].name).toBe('second.wav');
  // The stale clip's length warning is not shown for the clip the user kept.
