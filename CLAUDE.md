...
- Docker: `ghcr.io/debpalash/voicestudio:latest` = **main** (rolling preview); `:X.Y.Z` + `:X.Y` + `:stable` = tagged releases. `:latest` is the preview channel by design — stable users pin `:stable` or a version tag. The legacy `ghcr.io/debpalash/omnivoice-studio` coordinate receives the same tags during migration.
